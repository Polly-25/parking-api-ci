...
def create_app(test_config=None):
    ...
    from models import db, ClientParking, Client, Parking
    db.init_app(app)
    ...
    def add_new_parking_zone():
        try:
            ...
            new_parking_zone = Parking(address=address, opened=opened, count_places=count_places, count_available_places=count_available_places)
            db.session.add(new_parking_zone)
            db.session.commit()
            parking_id = new_parking_zone.id
            return jsonify({
                "id": parking_id,
                "address": new_parking_zone.address,
                "opened": new_parking_zone.opened,
                "count_places": new_parking_zone.count_places,
                "count_available_places": new_parking_zone.count_available_places
            }), 201
        except Exception as e:
            db.session.rollback()
            return jsonify({"error": str(e)}), 500
    ...
    def leave_the_parking_lot():
        ...
        parking_record = db.session.query(ClientParking).filter_by(
            client_id=client_id,
            parking_id=parking_id
        ).first()
